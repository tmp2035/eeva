...
"""
    ...
    but with confidence bounds provided by UCB
...
class PROPOSED_UCB(Cache):
    ...
    self.cacheline_deq [item_pos: (req_id, weight, last_call_step, table_size)]
    """

    def __init__(self, cache_size, alpha, beta, with_min=False, **kwargs):
        super(PROPOSED_UCB, self).__init__(cache_size, **kwargs)
        self.cacheline_list = -np.ones(shape=(cache_size, 5), dtype=float)
        self.table_weights = defaultdict(lambda: 1.0)
        self.table_medians = defaultdict(lambda: 1.0)
    ...
    def _normalize(self):
        ...
        if mm > self.max_value:
            ...
            for key in self.table_weights.keys():
                ...
                self.table_medians[key] = self.table_medians[key]/mm
    ...
    def _get_init_weight(self, req_item):
        ...
        return -0.2 * self.table_medians[table_num]* self.step + \
                1.2 * self.table_weights[table_num]#0.5 * (self.table_weights[table_num] + self.table_medians[table_num]* self.step)
    ...
    def _update(self, req_item, **kwargs):
        ...
        self.cacheline_list[list_pos][[WEIGHT,]] = np.array(
            [
    ...
    def _insert(self, req_item, w, **kwargs):
        ...
        if isinstance(req_item, Req):
            req_id = req_item.item_id
        _, table_num, _, table_size = parse_info(req_item.info)
        # check that hand in proper position
        assert int(self.cacheline_list[self.hand][0]) == -1

        self.cacheline_dict[req_id] = self.hand

        self.cacheline_list[self.hand] = np.array([req_id, w, self.step, table_size, table_num])
    ...
    def evict(
        ...
        if self.with_min:
            ...
            # bounds =  1. * (self.cacheline_list[:, SIZE] * self.cacheline_list[:, AGE])**0.5/ \
                        # (np.log(self.step)/self.step)
            victim = np.argmin(self.cacheline_list[:, WEIGHT]) # - bounds)
        else:
            # this is for EEvA
        ...
        victim_table = int(self.cacheline_list[victim][TABLE_NUM])

        tmp_w = self.cacheline_list[victim][WEIGHT]
        sgn = (tmp_w/self.step > self.table_medians[victim_table])
        self.table_medians[victim_table] = min(max(self.table_medians[victim_table] + (2 * sgn - 1.) * 0.0001, 0.0), 1.)
        # if self.step % 200 == 0:
            # print(victim_table, sgn, self.table_medians[victim_table]* self.step, self.table_weights[victim_table])
