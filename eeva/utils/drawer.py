...
def plot_dict(
    ...
    for i, (key, val) in enumerate(dct.items()):
        ...
        for j in range(1, 5):
            figs[j - 1][1].plot(
                val[:, 0], val[:, j], label=key, color=figs[j - 1][2][i], linestyle=l_style, linewidth=2
            )
    for i in range(4):
...
def plot_bar(exp):
    ...
    rez = pd.DataFrame(
        v, columns=["Algorithm", "size", "Miss rate", r"Averaged time cost, $C$", "Scan misses", "Get misses"]
    )
