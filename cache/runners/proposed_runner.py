...
def trace_reader(trace_file):
    with open(trace_file, "r") as f:
          r = f.readline() #линия с названиями столбцов
          while (s:= f.readline()) != "":
              yield s
...
def run(tracepath:str, cache_sizes: str, algorithms: str, alpha: float, beta: float, move_cost: float, return_trace_hit: bool = False, n_jobs = 12):
    ...
    if n_jobs > 1:
        ...
        comp_rez = Parallel(n_jobs= n_jobs, return_as="generator")(del_sim(elem, tracepath) 
                                                           for elem in lst)
    else:
