...
from ..utils.costs_calc import calculate_cost, count_miss_types
...
class Experiment:
    ...
    def from_config( params: Config, generator: Generator = None):
        ...
        if generator is None:
            generator = Generator(num_requests=gen_params.num_requests,
    ...
    def run_libCache(self): 
        if not Path(self.config.paths.lib_cache_sim).exists():
            ...
            return
        assert hasattr(self, "tracepath"), f"tracepath is not specified. Previously generate trace or specify by existing data"
    ...
    def save(self, name: str = "", description: str = "", rewrite = False):
        assert hasattr(self, "tracepath"), f"tracepath is not specified. Previously generate trace or specify by existing data"
        assert hasattr(self, "rez"), f"rezults is not computed, nothing to save"
