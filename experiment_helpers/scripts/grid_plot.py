...
def plot_dict(config, rez):
    ...
    rez = pd.DataFrame(v, columns=["Algorithm","p_scan", "Miss rate", "Averaged time cost", "Scan misses","Get misses"])
    disp = pd.DataFrame(d, columns=["Algorithm","p_scan", "Miss rate", "Averaged time cost", "Scan misses","Get misses"])
    # rez.p_scan = [f"{float(x)}" for x in rez.p_scan]
    rez = rez.astype({"Algorithm": str,"p_scan": float, "Miss rate": float, "Averaged time cost":float, "Scan misses":float,"Get misses":float})
    disp = disp.astype({"Algorithm": str,"p_scan": float, "Miss rate": float, "Averaged time cost":float, "Scan misses":float,"Get misses":float})
    ...
    for i, alg_name in enumerate(rez["Algorithm"].unique()):
        ...
        plt.show()
            ...
            ax[k].fill_between(r_tmp["p_scan"], m - d, m + d, color=colors[i], alpha=0.15, interpolate = True,)
    for i in range(2):
